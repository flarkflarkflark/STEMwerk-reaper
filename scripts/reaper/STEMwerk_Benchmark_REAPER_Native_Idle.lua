-- @description STEMwerk: UI Idle Benchmark - REAPER Native (LOCAL TEST ONLY - not shipped)
...
-- REAPER Native utility theme + visual FX disabled.
...
-- After dofile: SETTINGS, updateTheme are available.
-- Force FX off without saving to persistent storage.
if type(SETTINGS) == "table" then
    SETTINGS.visualFX = false
end
...
if lf then
    ...
    lf:write("# visual_fx=false  force_utility=true  theme=reaper_native\n")
...
local function benchmarkTick()
    ...
    if (now - bm.last_log_t) >= 5 and bm.log_path then
        ...
        local thm = (type(SETTINGS) == "table") and tostring(SETTINGS.themePreset or "reaper_native") or "?"
