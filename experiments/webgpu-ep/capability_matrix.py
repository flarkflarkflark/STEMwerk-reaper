...
MATRIX: list[CapabilityEntry] = [
    ...
    CapabilityEntry(
        os_arch="Linux x86_64 (Arch Linux, kernel 7.2.4-arch1-2)",
        gpu_vendor="NVIDIA", gpu_model="GeForce RTX 3060 Laptop GPU",
        gpu_device_id_hex="0x2520", gpu_pci_bus_id="0000:01:00.0",
        model_name="UVR_MDXNET_KARA_2.onnx",
        model_sha256="bf32e15105a09c0f7dddd2b67346146334d6f3ecb399ed7638eba2ab07cbf5f4",
        inference_backend="onnxruntime-ep-webgpu 0.3.0 (WebGpuExecutionProvider)",
        underlying_graphics_backend="Vulkan (NVIDIA proprietary ICD)",
        runtime_version="CPython 3.11.16; onnxruntime 1.30.0 + onnxruntime-ep-webgpu 0.3.0",
        theoretically_supported=True, actually_tested=True, found_correct=True, suitable_for_auto=True,
        actual_gpu_execution="PASS -- physical RTX 3060 execution proved by 185/185 WebGPU placement, "
                             "Vulkan/NVIDIA process maps and device nodes, named Dawn dispatches, and live "
                             "per-process nvidia-smi evidence; this proves execution, not selector causality",
        physical_gpu_verification="PASS -- the validation PID was independently attributed to the RTX 3060 "
                                  "by nvidia-smi (about 3548 MiB during Demucs), with NVIDIA ICD libraries and "
                                  "/dev/nvidia0 open. The requested PCI device may still have coincided with "
                                  "Dawn's default; explicit selector enforcement was NOT proved",
        graph_placement="185/185 nodes on WebGPU, 0 CPU fallback (N1 Section 6)",
        numerical_correctness="PASS -- raw CPU/WebGPU corr=1.0; max_abs_diff=4.77e-07 vocals / "
                              "4.47e-07 instrumental; exported WAV max diff one PCM16 LSB",
        end_to_end_audio_correctness="PASS -- full two-stem pipeline, export validation, and routing",
        stability="PASS -- controlled full pipeline plus four-run benchmark completed",
        practical_performance="Warm WebGPU 2.083s vs CPU EP 12.045s on the 20s fixture: 5.78x faster "
                              "(RTF 9.6x)",
        vendor_alternative_available=None,
        device_selection_enforceable="NOT PROVED BY N1 -- ordinary explicit PCI selection requested "
                                      "0000:01:00.0, but the physical-execution evidence cannot distinguish "
                                      "an honored request from Dawn choosing the same default. L10's separate "
                                      "VK_LOADER_DEVICE_ID_FILTER subprocess mechanism is available on Linux",
        evidence="DEMUCS_LINUX_NVIDIA.md Sections 2, 4, 6, and 9",
    ),
    CapabilityEntry(
        os_arch="Linux x86_64 (Arch Linux, kernel 7.2.4-arch1-2)",
        gpu_vendor="NVIDIA", gpu_model="GeForce RTX 3060 Laptop GPU",
        ...
        runtime_version="CPython 3.11.16; onnxruntime 1.30.0 + onnxruntime-ep-webgpu 0.3.0",
        theoretically_supported=True, actually_tested=True, found_correct=True, suitable_for_auto=True,
        actual_gpu_execution="PASS -- physical RTX 3060 execution proved by 1594/1594 WebGPU placement, "
                             "zero CPU fallback, Vulkan/NVIDIA process maps and device nodes, named Dawn "
                             "dispatches, and live per-process nvidia-smi evidence; selector causality unproved",
        physical_gpu_verification="PASS -- live nvidia-smi attributed about 3548 MiB to the validation PID, "
                                  "with NVIDIA ICD libraries and /dev/nvidia0 open. This proves the RTX ran "
                                  "the workload, not that the requested PCI selector caused that choice",
        graph_placement="1594/1594 nodes on WebGPU, 0 CPU fallback (N1 Section 7)",
        numerical_correctness="PASS -- shifts=2 across seeds 111/222/333: CPU/WebGPU corr>=0.999999996, "
                              "relative RMS error 0.0042%-0.0084%, max_abs_diff 8.17e-05 to 3.11e-04",
        end_to_end_audio_correctness="PASS -- full four-stem pipeline, WAV validation, and routing",
        stability="PASS -- ORT_ENABLE_BASIC required and sufficient; ORT_ENABLE_ALL reproduces the known "
                  "cross-backend ConvActivationFusion failure",
        practical_performance="Warm WebGPU 14.144s vs ONNX CPU 28.578s: 2.02x faster (RTF 1.41x), but "
                              "approximately 5.9x slower than the measured warm PyTorch/CUDA route (~2.399s)",
        vendor_alternative_available="PyTorch/CUDA -- STEMwerk's established production route on this exact "
                                     "Linux RTX 3060, proven about 5.9x faster than WebGPU (N1 Section 8)",
        device_selection_enforceable="NOT PROVED BY N1 -- same distinction as the MDX-Net row: physical RTX "
                                      "execution is proved, causal enforcement by the ordinary PCI selector is "
                                      "not. L10's loader-filter isolation is a separate Linux mechanism",
        evidence="DEMUCS_LINUX_NVIDIA.md Sections 2, 4, 7, 8, and 9",
    ),
    CapabilityEntry(
    ...
    CapabilityEntry(
        ...
        gpu_vendor="NVIDIA", gpu_model="GeForce RTX 3060 Laptop GPU",
        gpu_device_id_hex="0x2520", gpu_pci_bus_id=None,
        model_name="UVR_MDXNET_KARA_2.onnx",
        model_sha256="bf32e15105a09c0f7dddd2b67346146334d6f3ecb399ed7638eba2ab07cbf5f4",
        inference_backend="locally built onnxruntime-ep-webgpu 0.3.0+w5fix1 "
                          "(WebGpuExecutionProvider; W4 LUID patch plus W5 context-retention fix)",
        underlying_graphics_backend="D3D12",
        runtime_version="CPython 3.11; onnxruntime 1.30.0 + locally built "
                        "onnxruntime-ep-webgpu 0.3.0+w5fix1",
        theoretically_supported=True, actually_tested=True, found_correct=True, suitable_for_auto=True,
        actual_gpu_execution="PASS -- 185/185 MDX-Net nodes placed on WebGPU and completed; a separate dense "
                             "Conv workload completed 714 calls while OS counters attributed the process only "
                             "to the requested RTX 3060 DXGI LUID 64318 (W5)",
        physical_gpu_verification="PASS -- fresh-process Windows GPU Engine counters keyed independently through "
                                  "HKLM\\SOFTWARE\\Microsoft\\DirectX observed PID 6336 only on LUID 64318: "
                                  "2 nonzero 3D-engine samples, 38.65 pct peak, and no AMD-LUID sample (W5)",
        graph_placement="185/185 MDX-Net nodes on WebGPU, 0 CPU fallback (W5)",
        numerical_correctness="PASS -- raw CPU/WebGPU corr=1.00000000; max_abs_diff=1.10e-06 vocals / "
                              "1.03e-06 instrumental; exported WAV max diff one PCM16 LSB (W5)",
        end_to_end_audio_correctness="PASS -- full 25s stereo two-stem pipeline, export validation, and routing",
        stability="PASS -- dense Conv probe plus full MDX-Net pipeline completed in fresh processes",
        practical_performance="W5 validation run: 5.20s WebGPU vs 47.17s CPU; prior W1 four-run warm benchmark "
                              "measured 5.17x faster than CPU",
        ...
        device_selection_enforceable="PASS for this experimental native build (W5) -- the selected "
                                      "OrtHardwareDevice DXGI LUID is passed to Dawn's "
                                      "RequestAdapterOptionsLUID, the returned adapter LUID is verified, "
                                      "cross-GPU context reuse is rejected, and independent counters observed "
                                      "only requested LUID 64318 during the dense workload",
        evidence="WINDOWS_NATIVE_BUILD_VALIDATION.md (W5); README.md Phase W5; "
                 "DEMUCS_WINDOWS_NVIDIA.md (earlier W1 model/performance evidence)",
    ),
    CapabilityEntry(
        os_arch="Windows 11, x86_64",
        gpu_vendor="NVIDIA", gpu_model="GeForce RTX 3060 Laptop GPU",
    ...
    CapabilityEntry(
        ...
        model_sha256="bf32e15105a09c0f7dddd2b67346146334d6f3ecb399ed7638eba2ab07cbf5f4",
        inference_backend="locally built onnxruntime-ep-webgpu 0.3.0+w5fix1 "
                          "(WebGpuExecutionProvider) -- explicitly requested by DXGI LUID 59967",
        underlying_graphics_backend="D3D12",
        runtime_version="CPython 3.11; onnxruntime 1.30.0 + locally built "
                        "onnxruntime-ep-webgpu 0.3.0+w5fix1",
        theoretically_supported=True, actually_tested=True, found_correct=True, suitable_for_auto=False,
        actual_gpu_execution="PASS -- 185/185 MDX-Net nodes completed on WebGPU; a separate dense Conv "
                             "workload completed 618 calls while OS counters attributed this process only "
                             "to the requested AMD DXGI LUID 59967 (W5)",
        physical_gpu_verification="PASS -- fresh-process Windows GPU Engine counters keyed independently through "
                                  "HKLM\\SOFTWARE\\Microsoft\\DirectX observed PID 6608 only on AMD LUID "
                                  "59967: 3 nonzero 3D-engine samples, 59.37 pct peak, and no RTX-LUID sample",
        graph_placement="185/185 MDX-Net nodes on WebGPU, 0 CPU fallback (W5)",
        numerical_correctness="PASS -- raw CPU/WebGPU corr=1.00000000; max_abs_diff=1.07e-06 vocals / "
                              "1.01e-06 instrumental; exported WAV max diff one PCM16 LSB (W5)",
        end_to_end_audio_correctness="PASS -- full 25s stereo two-stem pipeline, export validation, and routing",
        stability="PASS for the controlled dense Conv and full MDX-Net validation runs; no extended soak run",
        practical_performance="W5 single validation run: 14.51s WebGPU vs 26.43s CPU. This is not a repeated "
                              "benchmark, so the iGPU remains excluded from Auto pending performance/stability "
                              "characterization",
        vendor_alternative_available=None,
        device_selection_enforceable="PASS for this experimental native build (W5) -- requested LUID 59967 "
                                      "was enforced through Dawn, the returned LUID was verified, and the "
                                      "decisive reversed-selection workload produced positive AMD-only "
                                      "per-process GPU-engine activity",
