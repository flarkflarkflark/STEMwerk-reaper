...
clipping qualification; see §10). Integrated into the shared experiment branch from
validation branch `validate/webgpu-amd-windows` @ `4ca1fcf68a5b6d373e4f94a3b021ec59af6f0cbe`
(worktree `P:\GIT\STEMwerk-worktrees\webgpu-amd-windows`); base
`11c616125e227be325ec5978fef3a1d2f9a18ac5` (= `origin/experiment/webgpu-ep` HEAD at the
time of validation). The original validation branch is preserved unchanged; the
stop-gate preflight document `AMD_WINDOWS_VALIDATION_STATUS.md` remains on that
branch only and is superseded by this report.
