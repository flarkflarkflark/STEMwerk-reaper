...
Value: validate caller-supplied catalog/component bytes and compatibility context without network, effects or trust decisions, producing a deterministic machine-readable `ResolutionPreview`. Scope: schema/structure validation, exact component selection, identity/version/kind/artifact/provenance projection, structural signature-envelope processing, declarative compatibility evaluation, canonical preview and `resolution_preview_digest`, with typed explained rejection. Packages: artifact, provenance, component, catalog, compatibility and resolution; the signature boundary is structural only. Requirements are realization-level mappings in `READINESS_TRACEABILITY.md`, not blanket ownership of FAIL-001..012. Excludes writes, filesystem reads, downloads, persistent state, generation construction, cryptographic signature verification, trust/revocation decisions, activation, helper, installer and consumer integration. Entry: SLICE-0 exit and P2/P4/P5/P6/P7/P8 plus explicit authorization. Exit: the exact fixture, fault, determinism, security, artifact and four-native-platform evidence in `component-manager/docs/SLICE_1_SCOPE.md` passes with `SLICE1_EXIT=PASS`. Native: four-platform evidence is required for the exit claim, although no native adapter is introduced. Rollback: remove read-only handlers. Risk: LOW.
