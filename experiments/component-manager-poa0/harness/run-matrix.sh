...
record() {
  ...
  jq -e -s 'all(.[];type=="object")' "$output" >/dev/null 2>&1 || json=FAIL
  error_code=$(jq -rs '[.[]|select(.error_code != null and .error_code != "")|.error_code][-1] // ""' "$output" 2>/dev/null)
  active=$(jq -r '.generation_id // ""' "$root/state/active" 2>/dev/null)
  receipt=valid
  ...
  journal=missing
  test -s "$root/state/journal/operations.jsonl" && jq -e -s 'all(.[];type=="object")' "$root/state/journal/operations.jsonl" >/dev/null 2>&1 && journal=valid
...
run_case() {
  ...
  case "$mode" in
    ...
    idem) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >"$output" 2>/dev/null || code=$?; test "$(find "$root/store/components" -name .stemwerk-component.json | wc -l)" = 2 || outcome=FAIL ;;
    ...
    kill_swap) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; first=$(jq -r .generation_id "$root/state/active"); POA_FAULT=kill_after_active_swap "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >"$output" 2>/dev/null && outcome=FAIL || code=$?; second=$(jq -r .generation_id "$root/state/active"); test "$first" != "$second" || outcome=FAIL ;;
    ...
    rollback) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; first=$(jq -r .generation_id "$root/state/active"); "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; "$base/bin/cm-$impl" rollback --root "$root" >"$output" 2>/dev/null || code=$?; test "$(jq -r .generation_id "$root/state/active")" = "$first" || outcome=FAIL ;;
    rebuild) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; mv "$root/state/state.db" "$root/state/state.db.quarantine"; "$base/bin/cm-$impl" state-rebuild --root "$root" >"$output" 2>/dev/null || code=$?; sqlite3 "$root/state/state.db" 'select count(*) from inventory' | grep -qx 2 || outcome=FAIL ;;
    ...
    active_run|two_stage) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; first=$(jq -r .generation_id "$root/state/active"); "$base/bin/cm-$impl" run-pin --root "$root" --run-id matrix --duration-ms 500 >"$output" 2>/dev/null & pid=$!; while ! find "$root/state/leases" -type f | grep -q .; do sleep 0.01; done; "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; wait "$pid" || code=$?; stages=$(jq -r 'select(.event=="run_stage")|.generation_id' "$output" | sort -u | wc -l); test "$stages" = 1 || outcome=FAIL; test "$(jq -r 'select(.event=="run_stage")|.generation_id' "$output" | head -1)" = "$first" || outcome=FAIL ;;
    lease_gc) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; "$base/bin/cm-$impl" run-pin --root "$root" --run-id gc --duration-ms 300 >"$output" 2>/dev/null & pid=$!; while ! find "$root/state/leases" -type f | grep -q .; do sleep 0.01; done; test -n "$(find "$root/state/leases" -type f -print -quit)" || outcome=FAIL; wait "$pid" || code=$? ;;
    ...
    reader) "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >/dev/null 2>/dev/null; "$base/bin/cm-$impl" install --root "$root" --catalog "$base/fixtures/catalog.json" >"$output" 2>/dev/null & pid=$!; while kill -0 "$pid" 2>/dev/null; do a=$(jq -r .generation_id "$root/state/active" 2>/dev/null); test -z "$a" || jq -e '.components|length==2' "$root/generations/$a/generation.json" >/dev/null || outcome=FAIL; done; wait "$pid" || code=$? ;;
  esac
